...
train = h2o.import_file( os.path.join(
  path, "../datasets/football.train2.csv") )
valid = h2o.import_file( os.path.join(
  path, "../datasets/football.valid2.csv") )
test = h2o.import_file( os.path.join(
  path, "../datasets/football.test2.csv") )
...
train["ScoreDraw"] = (
  (train["FTHG"] > 0) & (train["FTHG"] == train["FTAG"]) ).asfactor()
valid["ScoreDraw"] = (
  (valid["FTHG"] > 0) & (valid["FTHG"] == valid["FTAG"]) ).asfactor()
test["ScoreDraw"] = (
  (test["FTHG"] > 0) & (test["FTHG"] == test["FTAG"]) ).asfactor()
